...
class TripleAttentionSmarterFull(nn.Module):
    ...
    def forward(self, x, test=False):
        ...
        Y = (c @ d.transpose(-2, -1)) * self.scale
        Z = (e @ f.transpose(-2, -1)) * self.scale
...
class TransformerBlock(nn.Module):
    def __init__(self, d_model, d_mlp, d_head, num_heads, n_ctx, act_type, mode, model):
        ...
        if mode=='TripletFull':
                self.attn = TripleAttentionSmarterFull(self.model,d_model, num_heads)
        if mode=='Standard':
...
class Transformer(nn.Module):
    def __init__(self, d_vocab, d_model, d_mlp, d_head, num_heads, n_ctx, act_type, use_cache=False,use_ln=True, output='logits', mode='Standard'):
        super().__init__()
        self.cache = {}
        self.use_cache = use_cache
        self.embed = Embed(d_vocab, d_model)
        #self.pos_embed = PositionalEncoding1(d_model,n_ctx)
...
def gen_validation(num,p, mu=10):#50):
    #data=[]
    sqn=int(np.sqrt(num))
    #inds=np.arange(sqn)
    x=torch.zeros((mu*500,sqn**2))
...
pairs=[(9, 0.225), (10, 0.2), (11, 0.175), (12, 0.15), (13, 0.15), (14, 0.125), (15, 0.125), (16, 0.125), (17, 0.1), (18, 0.1), (19, 0.09), (20, 0.09), (21, 0.09), (22, 0.075), (23, 0.075), (24, 0.075), (25, 0.075), (26, 0.075), (27, 0.075), (28, 0.075)]
...
for size,prob in pairs:
    train_nodes.append(size)
    #print(size, prob)
    #train_sets.append(gen_train_test(node**2, seed))
    train_sets.append(gen_validation(size**2, prob,mu))
#print(size,prob)
n_ctx = size**2
test = gen_validation(9*9, 0.2,mu)
...
def executeTraining(d_model,modde,output='logits',batch_size=128):
    ...
    print('Start')
    model = Transformer(d_vocab=d_vocab, d_model=d_model, d_mlp=d_mlp, d_head=d_head, num_heads=num_heads, n_ctx=n_ctx, act_type=act_type, use_cache=False,output=output, mode=modde)
    ...
    run_name = f"Triangles_{int(time.time())}"
    ...
    for epoch in range(num_epochs):
        ...
        for o in range(len(train_nodes)):
            #dataloaders[o].sampler.set_epoch(epoch)
            for tokens,labels in dataloaders[o]:
                ...
                scheduler.step()
                optimizer.zero_grad()
        if epoch%100 == 0: print(f"{epoch}_{loss.item()}")#_{train_acc.item():.4f}_{test_acc.item():.4f}")
        ...
        if epoch%100 == 0:
            ...
            for example,label in testLoader:
                ...
                for gpu in range(len(pred)):
                     ...
                     if pred[gpu]<thr and labels[gpu]==1:
                         err1+=1  
            print('Validation error: 9 nodes:_'+str(err1/len(test)*c_valid+err0/len(test)*(1-c_valid)))
            gen_losses.append(err1/len(test)*c_valid+err0/len(test)*(1-c_valid))
    return emp_losses,gen_losses
...
for k in range(len(test)):
    if test[k][1]==1:
        c+=1
print(c/len(test))
save_dict = {'emp_loss':[],'gen_loss':[]}
for k in range(10):
    losses_train,losses_val = executeTraining(16,modde='Triplet',output='binary')#,output='logits') #250x50 examples. 2 layers
    save_dict['emp_loss'].append(losses_train[0])
    save_dict['gen_loss'].append(losses_val)
#save_dict = {'losses':emp_losses}

torch.save(save_dict, "TripletRedu-Droput03.pth")
'''print(f"Done")
for o in range(len(train_nodes)):
  plt.plot(np.arange(0,1000,100),losses[o],label='nodes='+str(train_nodes[o]))
